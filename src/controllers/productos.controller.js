...
exports.updateStock = async (req, res) => {
  try {
    ...
    const result = await Producto.updateStock(producto_id, cantidad);
...
exports.restoreStock = async (req, res) => {
  try {
    ...
    const result = await Producto.restoreStock(producto_id, cantidad);
    
    if (result === 0) {
      return res.status(404).json({ message: 'Producto no encontrado' });
    }
    
    res.status(200).json({ message: 'Stock restaurado correctamente' });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Error al restaurar stock' });
  }
};